...
def set_config():
    ...
    try:
        ...
        print("ERR loading gcs_download_config.ini", str(e))
    ...
    if gui:
        ...
        # Start GUI
        window = Tk()
        window.title("LONGARROW - GCS/S3 DOWNLOAD TOOL WITH BREAK-POINT RESUMING")
        ...
        Label(window, text="Credential Profile").grid(column=0, row=4, sticky='w', padx=2, pady=2)
        ...
        save_config_txt = Checkbutton(window, text="Save to gcs_download_config.ini", var=save_config)
...
# Download small file
def download_smallfile(srcfile):
    ...
    try:
        ...
        pload_bytes = len(getBody)
        pload_speed = size_to_str(int(pload_bytes/pload_time)) + "/s"
        with open(dir_and_key, 'wb') as wfile:
            wfile.write(getBody)
        print(f'\033[0;34;1m --->Complete\033[0m {srcfileKey} - {pload_speed}\033[0m small_file')
    except Exception as e:
        logger.warning(f'Fail to download {srcfileKey}. {str(e)}')
    return

# Download file main stream
...
if __name__ == '__main__':
    ...
    # Define s3 client
    s3_config = Config(max_pool_connections=max_pool_connections, retries={'max_attempts': MaxRetry})
    if endpoint_url == "AWS":
